...
def Main(Nowurl,NowNumber,Sleeptime):
    ...
    if (len(Downloadurl) == 1): #判断 是否存在1080p种子
        ...
        with open(mkdirPatch + name + '720p' + '.torrent', "wb") as code:
            code.write(r.content)
        #下载1080p资源
        DownTorrenturl = "http://thz5.net/forum.php?mod=attachment&" + Downloadurl[1]
        r = requests.get(DownTorrenturl)
        # print("正在下载种子文件！")
        with open(mkdirPatch + name + '1080p' + '.torrent', "wb") as code:
            code.write(r.content)
...
if __name__ == '__main__':
    ...
    url = 'http://thz5.net/thread-1969491-1-1.html' #开始爬的第一个页面
